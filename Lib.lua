local library = {} local TweenService = game:GetService(“TweenService”)
local CoreGui = game:GetService(“CoreGui”) local Players =
game:GetService(“Players”) local player = Players.LocalPlayer

– Cleanup existing if CoreGui:FindFirstChild(“redui”) then
CoreGui:FindFirstChild(“redui”):Destroy() end

– Internal helpers local function createUICorner(parent, radius) local c
= Instance.new(“UICorner”) c.CornerRadius = radius or UDim.new(0,6)
c.Parent = parent return c end

function library:Win(title) local UserInputService =
...
local tabButtons = Instance.new(“ScrollingFrame”) tabButtons.Size =
UDim2.new(0,120,1,-35) tabButtons.Position = UDim2.new(0,0,0,35)
tabButtons.BackgroundTransparency = 0.9 tabButtons.ScrollBarThickness =
4 tabButtons.CanvasSize = UDim2.new(0,0,0,0) – จะอัปเดตอัตโนมัติ
tabButtons.Parent = main

local tabLayout = Instance.new(“UIListLayout”) tabLayout.SortOrder =
Enum.SortOrder.LayoutOrder tabLayout.Padding = UDim.new(0,6)
tabLayout.Parent = tabButtons

– อัปเดต CanvasSize ให้เท่ากับขนาดเนื้อหา
tabLayout:GetPropertyChangedSignal(“AbsoluteContentSize”):Connect(function()
tabButtons.CanvasSize = UDim2.new(0, 0, 0,
tabLayout.AbsoluteContentSize.Y) end)
...
closeBtn.MouseButton1Click:Connect(function() – ย่อเมนูหลัก
TweenService:Create( main, TweenInfo.new(0.4, Enum.EasingStyle.Quad,
Enum.EasingDirection.Out), {Size = UDim2.new(0.5, 0, 0, 0), Position =
UDim2.new(0.5, 0, -0.2, 0)} ):Play()
...
– กดปุ่มวงรี hubToggle.MouseButton1Click:Connect(function()
TweenService:Create( hubToggle, TweenInfo.new(0.3,
Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0, 0,
0, 0), Position = UDim2.new(0.5, 0, -0.2, 0)} ):Play()
...
– Tabs system local tabs = {}

function tabs:Tab(name) assert(library.tabButtons and library.pages,
“Call library:Win(title) before creating tabs.”) – create tab button
local tabButton = Instance.new(“TextButton”) tabButton.Size =
UDim2.new(1, -10, 0, 28) tabButton.BackgroundColor3 =
Color3.fromRGB(50,50,50) tabButton.BackgroundTransparency = 0.6
tabButton.TextColor3 = Color3.fromRGB(255,255,255) tabButton.Font =
Enum.Font.SourceSans tabButton.TextSize = 16 tabButton.Text = name
tabButton.Parent = library.tabButtons createUICorner(tabButton,
UDim.new(0,6))
...
function newPage:Dropdown(title, items, callback) local container =
Instance.new(“Frame”) container.Size = UDim2.new(1, -10, 0, 30)
container.BackgroundTransparency = 1 container.LayoutOrder = 0
container.Parent = page
...
– Notification system (single, clean) local activeNotifs = {} function
library:Notifile(title, msg, duration) local gui =
CoreGui:FindFirstChild(“redui”) if not gui then return end if
#activeNotifs >= 3 then local oldest = table.remove(activeNotifs, 1) if
oldest and oldest.Destroy then oldest:Destroy() end end
...
library.tabs = tabs return library
